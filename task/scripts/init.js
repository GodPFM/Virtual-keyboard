          ...
          <div class="virtual-keyboard__container">
            <div class="virtual-keyboard__keys">
              <div class="virtual-keyboard__keyboard--first">
                <button class="virtual-keyboard__keyboard--btn esc" id="escape">
    ...
    <div class="information">
      <div class="information__wrapper">
        <ul class="information__list">
          <li class="information__item">Переключение языка - Shift + Alt</li>
          <li class="information__item">Используемые ES6 особенности: <ul>
              <li>JavaScript let</li>
              <li>JavaScript const</li>
              <li>Arrow Functions</li>
          </ul></li>
          <li class="information__item">Используется eslint</li>
        </ul>
      </div>
    </div>
    </main>;`;

const listOfButtons = document.querySelectorAll(
...
function switchLanguage() {
  listOfButtons.forEach((el) => {
    let button = el;
    if (lang === 'en') {
      let lowerSymbol = button.getAttribute('data-text-unshift-en');
      let upperSymbol = button.getAttribute('data-text-shift-en');
      if (upperSymbol != null) {
        button.querySelector('.virtual-keyboard__top').innerHTML = upperSymbol;
      }
      if (lowerSymbol != null) {
        button.querySelector('.virtual-keyboard__bottom').innerHTML = lowerSymbol;
      }
    }
    if (lang === 'ru') {
      let lowerSymbol = button.getAttribute('data-text-unshift-ru');
      let upperSymbol = button.getAttribute('data-text-shift-ru');
      if (upperSymbol != null) {
        button.querySelector('.virtual-keyboard__top').innerHTML = upperSymbol;
      }
      if (lowerSymbol != null) {
...
document.body.addEventListener('keyup', (e) => {
  if (e.code === 'ShiftLeft' || e.code === 'ShiftRight') {
    shift = false;
    if (document.querySelector('.virtual-keyboard__bottom.press') != null) {
      document.querySelector('.virtual-keyboard__bottom.press').classList.remove('press');
    }
  }
...
function addSymbol(element) {
  if (lang === 'en') {
    if (shift === false) {
      let symbol = element.getAttribute('data-text-unshift-en');
      if (symbol != null) {
        inputForm.value += symbol;
      }
    } else {
      let symbol = element.getAttribute('data-text-shift-en');
      if (symbol != null) {
        inputForm.value += symbol;
      }
    }
  ...
  if (lang === 'ru') {
    if (shift === false) {
      let symbol = element.getAttribute('data-text-unshift-ru');
      if (symbol != null) {
        inputForm.value += symbol;
      }
    } else {
      let symbol = element.getAttribute('data-text-shift-ru');
      if (symbol != null) {
        inputForm.value += symbol;
      }
    }
...
function changeShift(element) {
  if (
    element.classList.contains('shift-left')
    || element.classList.contains('shift-right')
  ) {
    shift = !shift;
    if (shift) {
      element.classList.add('click');
      element.querySelector('.virtual-keyboard__bottom').classList.add('press');
    } else {
      element.querySelector('.virtual-keyboard__bottom').classList.remove('press');
      element.classList.remove('click');
    }
  }
  if (element.classList.contains('alt-right') || element.classList.contains('alt-left')) {
    if (shift) {
      shift = !shift;
      if (document.querySelector('.shift-left .virtual-keyboard__bottom') != null) {
        document.querySelector('.shift-left .virtual-keyboard__bottom').classList.remove('press');
      }
